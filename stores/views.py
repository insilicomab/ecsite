...
class ProductListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        product_type_name = self.request.GET.get('product_type_name', None) # 何もない場合は None
        product_name = self.request.GET.get('product_name', None) # 何もない場合は None
        ...
        order_by_price = self.request.GET.get('order_by_price', 0) # 何もない場合は 0
        if order_by_price == '1':
...
def add_product(request):
    if request.is_ajax:
        product_id = request.POST.get('product_id') # ajaxで定義したproduct_id
        quantity = request.POST.get('quantity') # ajaxで定義したquantity
...
class CartItemsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for item in query.all():
            ...
            in_stock = True if item.product.stock >= item.quantity else False # productのストックが自身が入力した数以上であればTrue
            tmp_item = {
